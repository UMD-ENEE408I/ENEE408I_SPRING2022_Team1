...
void loop(){
  ...
  while(true){  
    ...
    if((adc1.readADC(6) < 500 && adc2.readADC(5) < 500 && adc1.readADC(5) < 500 && adc2.readADC(4) < 500 && adc1.readADC(4) < 500) || 
      ...
      int index = rec_Message.indexOf(','); // we can expect a rec_Message to look like "Right,Right,Forward,Right,WINNER\n"
      move_to_make = rec_Message.substring(0, index);
      rec_Message = rec_Message.substring(index+1, rec_Message.length());
      if(index < 0){
        move_to_make = "WINNER";
      }
      Serial.print(move_to_make);
      Serial.println();
      Serial.println(index);


      //now check rec_message and do turn
      if(move_to_make == "Forward"){
        ...
        enc2_value = enc2.read()*-1;
        enc1_value = enc1.read();
        desired_enc1_value = enc1_value + 395 - 105;
        desired_enc2_value = enc2_value + 395 - 105;
        twinky_one_speed = twinky_max; // twinky_max + .061;
        twinky_two_speed = twinky_max;
        while(enc1_value < desired_enc1_value || enc2_value < desired_enc2_value){
          current_time = millis();
